...
ylabel('v1')
if mu_plot
    title('mu(T,x,v), beta=0.9, c=2, lambda=0.5')
elseif mu_short_plot
...
% % hold on
% % s_2=10.8696*0.1^2;
% % mu=1/(2*pi*s_2)^(0.5)*exp(-(y).^2/(2*s_2))*delta_y;
% % mu=mu/sum(mu);
% % plot(y,mu,'-r')
% title('mu(T,y1), beta=1.1')
% xlabel('y1')
% ylabel('mu(T,y1)')
...
% hold on
% s_2=10.8696*0.1^2;
% mu1=1/(2*pi*s_2)^(0.5)*exp(-(y-0.2).^2/(2*s_2))*delta_y;
% mu2=1/(2*pi*s_2)^(0.5)*exp(-(y+0.2).^2/(2*s_2))*delta_y;
% mu=(mu1+mu2)/(sum(mu1)+sum(mu2));
% plot(y,mu,'-r')
% title('mu(T,y1), beta=0.4')
% xlabel('y1')
% ylabel('mu(T,y1)')
